...
- **All text must be black** — every `mxCell` with a `value` attribute must include `fontColor=#000000` in its style. Never use white or light text on light backgrounds.
- **All text labels must have white fill** — use `fillColor=#FFFFFF` on text elements, never `fillColor=none`. Transparent backgrounds are invisible in dark mode.
- **Use contrasting colors** — light pastel fills (e.g., `#dae8fc`, `#d5e8d4`, `#FFF8E1`) with darker borders. Text must always be readable against the fill.
- **Clean straight lines** — all edges must use `edgeStyle=orthogonalEdgeStyle;rounded=0;` for clean, straight orthogonal routing. No curved or rounded edges.
- **Avoid overlapping edges** — route cleanly with explicit waypoints (`Array as="points"`) when edges cross between swim lanes or distant nodes.
