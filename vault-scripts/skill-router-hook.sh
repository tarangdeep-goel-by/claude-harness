...
LINES=""
# Only nudge skills that are actually INSTALLED. Adopters don't get the SM-coupled data skills, so
# nudging toward an absent skill is pure noise — check the global skills dir AND the vault's local
# skills before adding. (The owner has all of them, so behavior there is unchanged.)
VAULT_ROOT="${CWD%%vault-work*}vault-work"
add() {  # add <skill-slug> <nudge-text> — skip silently if the skill isn't installed
  local slug="$1"; shift
  [ -e "$HOME/.claude/skills/$slug" ] || [ -e "$VAULT_ROOT/.claude/skills/$slug" ] || return 0
  LINES="$LINES
- $1"
}
...
m 'mixpanel|funnel|cohort|retention|\bdau\b|\bmau\b|segmentation|payment success|fds? booked|bonds? payment|mf orders?|onboarding (funnel|drop|flow)|drop.?off|conversion rate|event name|\bsm_[a-z]|\btd_[a-z]|\bsb_[a-z]|dynamic_image_tapped|dynamic_base_page|how many .*(booked|paid|issued|users)|unique users|\btraffic\b' \
  && add mixpanel-analytics "mixpanel-analytics — any event/funnel/cohort/metric. Read SKILL.md + references/ (event-dictionary, programmatic-api); never guess an event name (wrong variant = silently wrong data)."
...
m 'build (a )?(chart|funnel|report|dashboard)|add to (the |my )?(mixpanel )?(dashboard|board)|create a funnel in mixpanel|save (this )?(as )?(a )?report|update (my |the )?(mixpanel )?board|commit (the |a )?chart|mixpanel report' \
  && add mixpanel-chart-builder "mixpanel-chart-builder — building/committing a Mixpanel report. Follow spec→validate (observed-event-shapes)→preview→commit. If an event/id isn't observed yet, capture-journey first."
...
m 'metabase|\bsql\b|which table|table for|\bschema\b|\bdb ?2[0-9]\b|\bdb ?3[0-9]\b|user_referee|user_reward_transaction|user_fixed_deposit|run (this )?query|pull (the )?data|export .*(data|csv)|reward total|\baum\b|transaction (count|table|level)' \
  && add metabase-query "metabase-query — any table/schema/SQL. Confirm table + columns from the skill; never guess."
...
m 'play console|play store|app store|store listing|custom listing|product page|\bcvr\b|acquisition|\baso\b|app reviews?|\bratings\b|\bppo\b|utm source|search quer|a/b.*(listing|store|ppo)' \
  && add play-console "play-console — Play Console / App Store analytics (listings, CVR, acquisition, A/B/PPO, reviews). Use the skill's API patterns."
...
m 'flutter|stable_money_flutter|stable-flutter|build (a |the )?(widget|screen|page|bottom.?sheet)|implement (this|the) (design|screen|figma)|wire up the .* (component|widget|page)' \
  && add flutter-dev "flutter-dev — Flutter widget/screen build. MANDATORY: read the skill + Urvesh's playbook (≤10 files/PR, no API/nav/DS edits, trSafe + camelCase keys, branch from dev) BEFORE writing code."
...
m 'capture journey|new walkthrough|scrape (my |the )?events|scrape events for|record (the )?events|capture .*(session|journey)|screenshots folder|check .*recent events' \
  && add capture-journey "capture-journey — capturing a real app session into the event catalogue. Follow the skill (stream API + walkthrough + update observed-event-shapes)."
...
m 'transcribe|meeting notes from|process (this|the) (transcript|call|recording)|here.?s (the |what ).*(transcript|discussed)|summari[sz]e the call|raw transcript' \
  && add transcriber "transcriber — meeting transcript/recording → structured note. Use transcribe.sh + the transcriber flow (don't hand-summarize)."
...
m 'did .* refer|why .*(no|did ?n.?t).*(reward|referral)|referral for \+?[0-9]|investigate referral|cx .*(referral|reward)|control ?hub.*referral|clicked .*link.*nothing|referral (case|escalation)|why .*did ?n.?t .* get .* reward' \
  && add referral-investigation "referral-investigation — CX referral forensics (phone-pair reconciliation across Mixpanel + Metabase DB29). Run the skill's L1/L2 script; don't eyeball a verdict."
